...
init spritesUrl =
    let
        ...
        ( pixels, pixelsCmd ) =
            Elmo8.Layers.Pixels.init canvasSize

        ( text, textCmd ) =
            Elmo8.Layers.Text.init canvasSize

        ( sprites, spritesCmd ) =
            Elmo8.Layers.Sprites.init canvasSize spritesUrl
    in
...
update msg model =
    case msg of
        PixelsMsg pixelsMsg ->
            let
                ( pixels, cmd ) =
                    Elmo8.Layers.Pixels.update pixelsMsg model.pixels
            in
        ...
        TextMsg sms ->
            let
                ( text, cmd ) =
                    Elmo8.Layers.Text.update sms model.text
            in
        ...
        SpritesMsg spritesMsg ->
            let
                ( sprites, cmd ) =
                    Elmo8.Layers.Sprites.update spritesMsg model.sprites
            in
